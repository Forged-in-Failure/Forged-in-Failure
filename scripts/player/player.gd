...
const FOOTSTEP_MIN_INTERVAL = 0.45
# --- Movement ---
...
# Height threshold: below this fall distance plays Land, above plays Thud
const LAND_MAX_FALL = 2.5

# --- Audio players ---
@onready var footstep_player: AudioStreamPlayer3D = $Footsteps
@onready var jump_player: AudioStreamPlayer3D = $JumpAudio
@onready var wind_player: AudioStreamPlayer3D = $WindAudio
@onready var thud_player: AudioStreamPlayer3D = $ThudAudio
@onready var land_player: AudioStreamPlayer3D = $LandAudio

# Sound variation count
const SOUND_VARIATIONS = 4

# --- Anti-repeat tracking ---
var last_sound_index: Dictionary = {}
...
func _ready():
	...
	if not is_xr:
		cam_orig_pos = cam.transform.origin
		fov_orig = cam.fov
...
func _physics_process(delta: float) -> void:
	footstep_cooldown = max(0.0, footstep_cooldown - delta)
	...
	if Input.is_action_just_pressed("ui_accept") and is_on_floor():
		...
		play_random_sound(jump_player, "res://Sounds/Human/Jump, Land/Jump/")
	...
	if not is_first_person and not is_xr:
	...
	if was_airborne and is_on_floor():
		...
		elif fall_distance > 0.0:
			play_random_sound(land_player, "res://Sounds/Human/Jump, Land/Land/")
		was_airborne = false
...
func play_random_sound(player: AudioStreamPlayer3D, folder_path: String) -> void:
	if player == null:
		return
	var last = last_sound_index.get(folder_path, -1)
	var index = randi_range(1, SOUND_VARIATIONS)
	if SOUND_VARIATIONS > 1:
		while index == last:
			index = randi_range(1, SOUND_VARIATIONS)
	last_sound_index[folder_path] = index
	var path = folder_path + str(index) + ".mp3"
...
func play_random_footstep() -> void:
	if footstep_cooldown > 0.0:
		return
	play_random_sound(footstep_player, "res://Sounds/Human/Footsteps/Concrete/")
	var interval := FOOTSTEP_MIN_INTERVAL
	if is_sprinting:
		interval /= 1.5
	footstep_cooldown = interval
...
func trigger_thud(strength: float):
	...
	play_random_sound(thud_player, "res://Sounds/Human/Thud/")
